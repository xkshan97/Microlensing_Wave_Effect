...
max_mass = '50_Chabrier'

M_sun = 1.9884099 * 10**30
G = 6.6743*10**(-11)
c = 2.9979246*10**8

kappa_set = [0.5967729088673934, 0.4038037265674532]
gamma_set = kappa_set
kappa_s_set = [0.2786377498329086, 0.27675318987558506]
LensRedshift = 0.5


for ImageIndex in tqdm(range(50)):
    for subImageIndex in range(2):
        
        kappa = kappa_set[subImageIndex]
        gamma = gamma_set[subImageIndex]
        kappaStar_stellar = kappa_s_set[subImageIndex]
        

        try:
            # f0=open('./MicroField_88/AveMassAndNum_' + str(round(kappa, 2)) + '_' + str(round(gamma, 2)) + '_' + str(round(1.1 * kappaStar_stellar, 2)) + '_' + str(round(LensRedshift, 2)) + '_%.2f.bin',"rb")
            f0=open('./MicroField_' + max_mass + '/AveMassAndNum_%.2f_%.2f_%.2f_%.2f_%.2f.bin'%(kappa, gamma, 1.1*kappaStar_stellar, LensRedshift, ImageIndex),"rb")
            ...
            if 1 - kappa - gamma > 0:                
                ...
                f1=open('./ResultMinimum_' + max_mass + '/TimeLength_min_%.2f_%.2f_%.2f_%.2f_%.2f.bin'%(kappa, gamma, 1.1*kappaStar_stellar, LensRedshift, ImageIndex),"rb")
                TimeLengthFile_min=struct.unpack("l"*1, f1.read(8*1))
                f1.close()
                TimeLengthFile_min = np.array(TimeLengthFile_min)
                fileArea = "./ResultMinimum_" + max_mass + "/adptive_Area_min_%.2f_%.2f_%.2f_%.2f_%.2f.bin"%(kappa, gamma, 1.1*kappaStar_stellar, LensRedshift, ImageIndex)
                fileTime = "./ResultMinimum_" + max_mass + "/adptive_Time_min_%.2f_%.2f_%.2f_%.2f_%.2f.bin"%(kappa, gamma, 1.1*kappaStar_stellar, LensRedshift, ImageIndex)
                ...
                """去掉尾巴"""
                length = len(time_raw)
                time_raw = time_raw[0:length*3//5]
                Ft_raw = Ft_raw[0:length*3//5]
                ...
                while Ft_raw[-1] < - 10 ** 4 and time_raw[-1] > 10 ** (-2):
                    time_raw = np.delete(time_raw, -1)
                    Ft_raw = np.delete(Ft_raw, -1)
                ...
                np.savetxt('./Freq_Time_Domain_Result_' + max_mass + '/Sta_Minimum_time_%.2f_%.2f_%.2f_%.2f_%.2f.csv'%(kappa, gamma, 1.1*kappaStar_stellar, LensRedshift, ImageIndex),time_new,delimiter=',')
                np.savetxt('./Freq_Time_Domain_Result_' + max_mass + '/Sta_Minimum_Ft_%.2f_%.2f_%.2f_%.2f_%.2f.csv'%(kappa, gamma, 1.1*kappaStar_stellar, LensRedshift, ImageIndex),Ft_new,delimiter=',')
                ...
                Ffsgn = constant * cw 
                # Ff = (Ff + Ffsgn) * np.exp(complex(0,1)*omegafreq*time_zero) #消除开始时间
                Ff = Ff + Ffsgn #不消除开始时间
                ...
                np.savetxt('./Freq_Time_Domain_Result_' + max_mass + '/Sta_Minimum_Ffabs_%.2f_%.2f_%.2f_%.2f_%.2f.csv'%(kappa, gamma, 1.1*kappaStar_stellar, LensRedshift, ImageIndex),Ffabs,delimiter=',')
                np.savetxt('./Freq_Time_Domain_Result_' + max_mass + '/Sta_Minimum_ThetaF_%.2f_%.2f_%.2f_%.2f_%.2f.csv'%(kappa, gamma, 1.1*kappaStar_stellar, LensRedshift, ImageIndex),ThetaF,delimiter=',')
            else:
                
                f0=open('./ResultSaddle_' + max_mass + '/X1020New_%.2f_%.2f_%.2f_%.2f_%.2f.bin'%(kappa, gamma, 1.1*kappaStar_stellar, LensRedshift, ImageIndex),"rb")
                ...
                f1=open('./ResultSaddle_' + max_mass + '/TimeLength_sad_%.2f_%.2f_%.2f_%.2f_%.2f.bin'%(kappa, gamma, 1.1*kappaStar_stellar, LensRedshift, ImageIndex),"rb")
                TimeLengthFile_min=struct.unpack("l"*1, f1.read(8*1))
                f1.close()
                TimeLengthFile_min = np.array(TimeLengthFile_min)
                fileArea = "./ResultSaddle_" + max_mass + "/adptive_Area_sad_%.2f_%.2f_%.2f_%.2f_%.2f.bin"%(kappa, gamma, 1.1*kappaStar_stellar, LensRedshift, ImageIndex)
                fileTime = "./ResultSaddle_" + max_mass + "/adptive_Time_sad_%.2f_%.2f_%.2f_%.2f_%.2f.bin"%(kappa, gamma, 1.1*kappaStar_stellar, LensRedshift, ImageIndex)
                ...
                Ft_raw = Area[0:-1] / (time[1::] - time[0:-1])
                time_zero = time[np.where(Ft_raw == np.max(Ft_raw))]
                time_raw = time[0:-1]
                ...
                np.savetxt('./Freq_Time_Domain_Result_' + max_mass + '/Sta_Saddle_time_%.2f_%.2f_%.2f_%.2f_%.2f.csv'%(kappa, gamma, 1.1*kappaStar_stellar, LensRedshift, ImageIndex),time_new,delimiter=',')
                np.savetxt('./Freq_Time_Domain_Result_' + max_mass + '/Sta_Saddle_Ft_new_%.2f_%.2f_%.2f_%.2f_%.2f.csv'%(kappa, gamma, 1.1*kappaStar_stellar, LensRedshift, ImageIndex),Ft_subtract,delimiter=',')
                ...
                plt.figure(5)
                plt.semilogx(freq, Ffabs / mu, label = 'Sign') #//FIXME
                ...
                np.savetxt('./Freq_Time_Domain_Result_' + max_mass + '/Sta_Saddle_Ffabs_%.2f_%.2f_%.2f_%.2f_%.2f.csv'%(kappa, gamma, 1.1*kappaStar_stellar, LensRedshift, ImageIndex),Ffabs,delimiter=',')
                np.savetxt('./Freq_Time_Domain_Result_' + max_mass + '/Sta_Saddle_ThetaF_%.2f_%.2f_%.2f_%.2f_%.2f.csv'%(kappa, gamma, 1.1*kappaStar_stellar, LensRedshift, ImageIndex),ThetaF,delimiter=',')
        except FileNotFoundError:
            print('FileNotFoundError ImageIndex = ' + str(ImageIndex) + ' subImageIndex = ' +  str(subImageIndex))
            pass
...
plt.ylabel('F(t)')
plt.grid()
plt.savefig('./Freq_Time_Domain_Result_' + max_mass + '/Sta_Minimum_Ft.png',dpi=450)
...
plt.ylabel('|F(f)|')
# plt.ylim(0,2.5)
plt.savefig("./Freq_Time_Domain_Result_" + max_mass + "/Sta_Minimum_Ffabs.png",dpi=450)

plt.figure(3)
plt.grid()
plt.xlabel('f[Hz]')
plt.ylabel(r'$\theta_F$')
plt.savefig("./Freq_Time_Domain_Result_" + max_mass + "/Sta_Minimum_ThetaF.png",dpi=450)
...
plt.ylabel('dA/dt')
plt.grid()
plt.savefig('./Freq_Time_Domain_Result_' + max_mass + '/Sta_Saddle_Ft.png',dpi=450)
...
plt.ylabel('|F(f)|')
# plt.ylim(0,2.5)
plt.savefig("./Freq_Time_Domain_Result_" + max_mass + "/Sta_Saddle_Ffabs.png",dpi=450)

plt.figure(6)
plt.grid()
plt.xlabel('f[Hz]')
plt.ylabel(r'$\theta_F$')
plt.savefig("./Freq_Time_Domain_Result_" + max_mass + "/Sta_Saddle_ThetaF.png",dpi=450)
